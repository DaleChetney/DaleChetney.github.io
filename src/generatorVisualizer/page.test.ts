...
/** Labels of the lattice nodes outlined as the classes that generated the group. */
const generating = (): string[] =>
  Array.from(document.querySelectorAll(".lattice-node.generating text")).map(
    (text) => text.textContent ?? "",
  );
/** Labels of the lattice nodes outlined as what the current selection generates. */
const generated = (): string[] =>
  Array.from(document.querySelectorAll(".lattice-node.generated text")).map(
    (text) => text.textContent ?? "",
  );
/** How many covers lie inside the sublattice the current selection generates. */
const generatedEdges = (): number =>
  document.querySelectorAll(".lattice-edges line.generated").length;
...
describe("groups page", () => {
  ...
  it("opens a second section, grouped by conjugate, when C_4 is selected", () => {
    ...
    expect(generating().sort()).toEqual(["C₆", "₃C₄"].sort());
  });
  ...
  it("outlines what the selection generates and marks the covers beneath it", () => {
    groupRow(DEFAULT.label).click();
    // C₆ is open with its first generator drawn: C₆ is generated, and the four
    // covers under it (C₆ over C₂ and C₃, each over C₁) lie inside it.
    expect(generated()).toEqual(["C₆"]);
    expect(generatedEdges()).toBe(4);
    latticeNode("₃C₄").dispatchEvent(new MouseEvent("click"));
    // C₄ and C₆ together generate the group, so now the whole lattice is.
    expect(generated()).toEqual([DEFAULT.displayName]);
    expect(generatedEdges()).toBe(7);
    latticeNode("C₆").dispatchEvent(new MouseEvent("click"));
    expect(generated()).toEqual(["₃C₄"]);
    expect(generatedEdges()).toBe(2);
    latticeNode("₃C₄").dispatchEvent(new MouseEvent("click"));
    expect(generated()).toEqual([]);
    expect(generatedEdges()).toBe(0);
  });
